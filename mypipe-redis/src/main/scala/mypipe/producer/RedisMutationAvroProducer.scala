 ...
 *  as Avro records and publishes them into Redis.
 *
 */
abstract class RedisMutationAvroProducer[SchemaId](config: Config)
  ...
  protected val redisConnect = config.getString("redis-connect")
  protected val producer = new RedisProducer[OutputType](redisConnect)
   ...
   */
  protected def getRedisTopic(mutation: Mutation): String
  ...
  override def queueList(inputList: List[Mutation]): Boolean = {
    inputList.foreach(input ⇒ {
      ...
      records foreach (record ⇒ {
        ...
        producer.send(getRedisTopic(input), jsonString)
      })
    })
  ...
  override def toString(): String = {
    s"redis-avro-producer-$redisConnect"
  }
