...
abstract class RedisMutationAvroConsumer[InsertMutationType <: SpecificRecord, UpdateMutationType <: SpecificRecord, DeleteMutationType <: SpecificRecord, SchemaId](
  topic: String,
  redisConnect: String,
  groupId: String,
  schemaIdSizeInBytes: Int)(insertCallback: (InsertMutationType) ⇒ Boolean,
    ...
    extends RedisConsumer(topic, redisConnect, groupId) {
  ...
  override def onEvent(bytes: Array[Byte]): Boolean = true //try {
